...
local whatsapp_command = function ()
    helpers.run_or_raise({instance = 'web.whatsapp.com'}, false, 'chromium --app=https://web.whatsapp.com --new-window')
end
local discord_command = function ()
    -- Run or raise Discord running on the browser, spawned with Chromium browser's app mode
    -- helpers.run_or_raise({instance = 'discordapp.com__channels_@me'}, false, 'chromium --app=https://discordapp.com/channels/@me --user-data-dir=$HOME/.config/chromium/Profile 1 --new-window')
    -- Run or raise Discord app
    helpers.run_or_raise({class = 'discord'}, false, 'discord')
end
local mail_command = function ()
    helpers.run_or_raise({instance = 'mail.google.com'}, false, user.email_client, {switchtotag = true})
end
